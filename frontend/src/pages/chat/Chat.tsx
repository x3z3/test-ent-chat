              ...
              <Stack horizontal className={styles.chatInput}>
                ...
                <QuestionInput
                  ...
                  onFeedbackSubmit={async (feedback) => {
                    if (feedback) { // Check if feedback is not null
                      console.log(feedback);
                      const trimmedMessages = messages.filter(message => {
                        const isEmptyContent = !message.content || message.content.trim() === '';
                        const isEmptyObject = Object.keys(message).length === 0;
                    
                        return !isEmptyContent && !isEmptyObject;
                      });
                    
                      try {
                        const response = await submitConversationFeedback(feedback, messages);
